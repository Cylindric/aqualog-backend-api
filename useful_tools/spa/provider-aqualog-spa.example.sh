...
required_vars=(CLIENT_ID REFRESH_TOKEN)
for var in "${required_vars[@]}"; do
  if [[ -z "${!var:-}" ]]; then
    echo "Missing required environment variable: $var"
    echo "Export values before running this script, or source a provider helper file."
    exit 1
  fi
done
...
echo "== 1) Exchange refresh token =="
RESP="$(request_refresh_token "$REFRESH_TOKEN")"
...
ROTATED_REFRESH_TOKEN="$(echo "$RESP" | jq -r '.refresh_token // empty')"
NEXT_REFRESH_TOKEN="${ROTATED_REFRESH_TOKEN:-$REFRESH_TOKEN}"

echo "== 5) Test refresh-token reuse/rotation =="
RESP2="$(request_refresh_token "$NEXT_REFRESH_TOKEN")"
...
JWT2="$(echo "$RESP2" | jq -r '.id_token // .access_token // empty')"
if [[ -n "$JWT2" ]]; then
  echo "== 6) Call API with refreshed token =="
  curl -s -H "Authorization: Bearer $JWT2" "$API_URL" | jq .
fi