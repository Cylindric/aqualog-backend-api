...
Define OAuth2/OIDC authentication behavior for API endpoint protection, token validation, and authentication error handling.

## ADDED Requirements

### Requirement: OAuth2 token validation is available for endpoint protection
The system SHALL provide OAuth2 bearer token validation that can be applied to API endpoints.
...
- **THEN** the token is validated and the request proceeds to the endpoint handler
